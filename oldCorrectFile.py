...
class PPOMemory:
    ...
    np.array(self.buffer_returns),np.array(self.buffer_entropy),batches

    def remember(self,state,actions,reward,next_state,done,log_probs,state_value,entropy):
        ...
        self.buffer_entropy.append(entropy)
...
class Agent:
    def __init__(self,gamma = 0.99,policy_clip = 0.2,batch_size = 10,epochs = 5,n_joints = 8,input_size = 23,output_size = 8,N = 50):
    ...
    def remember(self,state,actions,reward,next_state,done,log_probs,state_value,entropy):
        self.memory.remember(state,actions,reward,next_state,done,log_probs,state_value,entropy)
    ...
    def learn(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            advantages, returns,entropies, batches = self.memory.generate_batches()
            ...
            for batch in batches:
                ...
                t_entropy = torch.tensor(entropies[batch])
...
def train():
    ...
    while True:
        ...
        final_move,action_probs,entropy,state_value = agent.choose_actions(state = torch.tensor(state_old,dtype=torch.float))
        ...
        agent.remember(state=state_old,actions=final_move,reward=reward,
                       next_state=state_new,done=done,state_value=state_value,log_probs=action_probs.detach().numpy(),entropy = entropy.detach().numpy())
        if done:
